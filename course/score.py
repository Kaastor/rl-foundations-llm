...
# Treat this as a contract: if you change it, change tests and bump SCORER_VERSION.
REWARD_SPEC = {
...
def _extract_example_fields(example: Union[Example, Mapping[str, Any]]) -> tuple[str, str, Optional[int], Dict[str, Any]]:
    ...
    try:
        ...
        return "<example-extraction-failed>", "", None, {"example_error": {"code": "example_extraction_failed", "message": str(e)}}
...
def _parse_final_line(line: str) -> tuple[Optional[int], Dict[str, Any]]:
    ...
    # Disallow plus sign and any non-digit characters (except a leading '-').
    if rest[0] == "+":
    ...
    # At this point, it's safe to parse.
    try:
        value = int(rest)
    except Exception as e:  # pragma: no cover (defensive)
...
def score(example: Union[Example, Mapping[str, Any]], completion: Any) -> Dict[str, Any]:
    ...
    """
    ex_id, prompt, expected, ex_extra = _extract_example_fields(example)
