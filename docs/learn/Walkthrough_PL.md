...
```bash
python -m course.bandit_train --steps 30 --seed 0 --lr 0.5 --baseline --slow --outdir runs/demo_bandit_slow
```
...
```bash
python -m course.bandit_train --steps 30 --seed 0 --lr -0.5 --baseline --slow --outdir runs/demo_bandit_negative
```
...
```bash
python --version
python -c "import course; print('Course package available')"
ls data/datasets/
ls data/rollouts/
```
...
```bash
python -m course.inspect_run --run <run_dir> --top-k 5 --show 2
python -m course.inspect_run --run <run_dir> --only-fails --top-k 5 --show 2
```
